...
def helper_test_missing_var(client, attr_dict, var):
    ...
    response = client.get(get_query_string(attr_dict_new))
...
def helper_test_valid(client, attr_dict):
    attr_dict_new = dict(attr_dict)
    get_query_string(attr_dict_new)
    response = client.get(get_query_string(attr_dict_new))
    assert response.status_code == 200
...
def test_extra_income(client) : helper_test_extra_var(client, attr_dict, "income", 20)
